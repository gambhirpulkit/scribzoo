...
$select_writer = mysql_query("select pid,title,r_email,s_email,name,description,imgname,video_id,allowed,receiver_string from posts where s_email = '{$_SESSION['SESS_EMAIL']}' order by pid desc limit 1");
...
<div class="widget gridalicious-item not-responsive " >
    ...
    <div class="bg-gray  border-bottom innerAll">
		...
		<input type="text" id="title_edit" value="<?php echo $result['title']; ?>" style="display:none"/>
		 ...
		 &nbsp; &nbsp; Posted by <a href="#"><?php echo $_SESSION['SESS_FIRST_NAME'].' '. $_SESSION['SESS_LAST_NAME']; ?></a>&nbsp;for&nbsp;<?php if($result['r_email'] == ''){echo $result['name']; }else{?><a href="profile.php?user=<?php echo $result['r_email']; ?>" target="_blank" class="no-ajaxify"><?php echo $result['name']; ?> </a> <?php } ?>&nbsp;with 
...
<iframe src="testimonial_image_upload.php" width="550" height="320" frameBorder="0"></iframe>
