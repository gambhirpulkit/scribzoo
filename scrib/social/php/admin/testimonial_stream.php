...
$qry=mysql_query("select stream_id from posts where pid = '$pid'");
$result=mysql_fetch_array($qry);
$allowed_user = explode(",", $result['stream_id']);

if (in_array($visitor_id,$allowed_user))
{?>	
...
// we are getting email adress from session here
 ...
 <a href="#" return false; id="before_sharing" style="float:right" onclick="share('<?php echo $pid ?>','<?php echo $_SESSION['SESS_MID'] ?>'); update_count_share();" data-original-title="share this testimonial"></i><h5>&nbsp;&nbsp;Share &nbsp;</h5></a>
...
//getting session email from session
		   ...
		   <a href="#" id="before_voting" style="float:right" onClick="vote_up('<?php echo $pid ?>','<?php echo $_SESSION['SESS_MID']?>'); update_count();" data-rel="tooltip" data-original-title="Vote up this testimonial"></i><h5>&nbsp;&nbsp; Vote Up </h5></a>
<p id="after_voting" onClick="unvote('<?php echo $pid ?>','<?php echo $_SESSION['SESS_MID'] ?>');update_count_reverse()" style="float:right;display:none">You voted this up &nbsp;<a href="#" >Unvote</a></p>
 <?php }else{ ?>
							   ...
							   <p id="after_unvoting" style="float:right;">You voted this up &nbsp;<a href="#" onClick="unvote('<?php echo $pid ?>','<?php $_SESSION['SESS_MID'] ?>');update_uncount_reverse()" title="Unvote this testimonial" data-rel="tooltip">Unvote</a></p> 
...
$newDate = date("d-m-Y", strtotime($originalDate));
    ...
    <div class="innerAll border-top">
	<?php echo"You are not allowed to view this testimonial.If you want to view this testimonial you have to send request to request to".' '?>
<a href="profile.php?user="<?php echo $mid; ?> ><?php echo $result['name']; ?></a>
